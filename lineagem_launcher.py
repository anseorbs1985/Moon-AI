...
def hover_click(x, y, wait=(0.35, 0.65)):
    """**마우스를 그 자리에 올려놓고 잠깐 뒤에 누른다** (과거섬 방식).

    버튼이 '커서가 올라와야' 활성화되는 경우가 있어서, 바로 누르면 씹힌다.
    계정전환 팝업의 [확인] 이 그런 자리다 (2026-08-29 사용자 요청)."""
    try:
        pyautogui.moveTo(int(x), int(y))
        time.sleep(random.uniform(*wait))
        pyautogui.moveTo(int(x), int(y))      # 한 번 더 — 확실히 올라가 있게
...
ZONE_OUT_MIN = 0.60  # 구역 **밖**에서 찾을 때는 이 점수를 넘어야만 인정한다.
                     # 구역 밖은 엉뚱한 곳일 확률이 높아 느슨한 판정(배수·흑백)을 쓰지 않는다.
                     # 0.75 → 0.60 (2026-09-02 실측). 0.75 는 **진짜 마름모 3개를 버렸다**
                     # (증거사진 17장 중 0.70~0.75 구간에 3장). 잡음 천장은 0.45 라
                     # (16클라 × 구역·창전체 32건, 크기보정 포함) 0.60 이면 여유 0.15 · 오탐 0.
                     # 사용자 지적: "화면에 그거 하나뿐이라 기준을 낮춰도 된다" — 맞는 말이다.
                     # 0.55 밑으로는 내리지 말 것 (여유가 0.10 밑으로 떨어진다).
...
class App(tk.Tk):
    ...
    # 좌표는 **번호 순서대로** 누른다. 칸마다 그림(🖼)과 간격(초)을 따로 줄 수 있다.
    # 저장 형태: [dx, dy, w0, h0] — 등록할 때의 **창 크기(w0,h0)도 함께** 남긴다.
